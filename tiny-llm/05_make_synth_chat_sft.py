#!/usr/bin/env python3
"""
Build a 200k+ synthetic SFT chat dataset (instruction/output) from:
- OpenAssistant/oasst1 (leaf assistant replies)
- databricks/databricks-dolly-15k
- deterministic synthetic templates (chatty + math + simple tech)

Output: data/synth_chat_sft.jsonl
"""
import json
import os
import random
...
TARGET_OASST = 100_000
TARGET_DOLLY = 40_000
# synthetic will fill the remainder
...
def build_synthetic_pairs(target: int):
    ...
    for a in range(10, 210, 3):
        b = 7
        prompts.append((f"What is {a}-{b}?", f"{a}-{b} = {a - b}."))
    for a in range(3, 103):
        b = 4
        prompts.append((f"What is {a}*{b}?", f"{a}*{b} = {a * b}."))
    for a in range(20, 420, 5):
        b = 5
        prompts.append((f"What is {a}/{b}?", f"{a}/{b} = {a // b}."))

    # expand to target with slight variations
    pairs = []
    i = 0
...
def main():
    ...
    with open(OUT_PATH, "w", encoding="utf-8") as f:
        for instr, out in pairs:
            f.write(json.dumps({"instruction": instr, "output": out}, ensure_ascii=False) + "\n")
